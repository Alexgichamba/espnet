...
class UTMOS22Strong(nn.Module):
    ...
    def forward(
        ...
        # first, pool the frame-level feature to utterance-level feature with mean pooling
        feat_utterance = feat_series.mean(dim=1, keepdim=True)
        if return_feat:
            # return a frame-level feature
            return feat_utterance
