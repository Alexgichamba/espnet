...
if [ -z ${data_dir_prefix} ]; then
    log "Root dir for dataset not defined, setting to ${MAIN_ROOT}/egs2/spoofceleb"
    data_dir_prefix=${MAIN_ROOT}/egs2/spoofceleb
else
    log "Root dir set to ${SPOOFCELEB}"
    data_dir_prefix=${SPOOFCELEB}
fi

if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    log "stage 1: SpoofCeleb Training Data Preparation"

    if [ ! -d "${trg_dir}/spoofceleb_train" ]; then
        log "Making Kaldi style files for train"
        mkdir -p "${trg_dir}/spoofceleb_train"
        python3 local/train_data_prep.py "${data_dir_prefix}/spoofceleb" "${trg_dir}/spoofceleb_train"
        for f in wav.scp utt2spk utt2spf; do
            sort ${trg_dir}/spoofceleb_train/${f} -o ${trg_dir}/spoofceleb_train/${f}
        done
        utils/utt2spk_to_spk2utt.pl ${trg_dir}/spoofceleb_train/utt2spk > "${trg_dir}/spoofceleb_train/spk2utt"
        utils/utt2spk_to_spk2utt.pl ${trg_dir}/spoofceleb_train/utt2spf > "${trg_dir}/spoofceleb_train/spf2utt"
        utils/validate_data_dir.sh --no-feats --no-text "${trg_dir}/spoofceleb_train" || exit 1
    else
        log "${trg_dir}/spoofceleb_train exists. Skip making Kaldi style files for train"
    fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    log "stage 2: SpoofCeleb Development and Evaluation Data Preparation"

    for split in dev eval; do
        if [ ! -d "${trg_dir}/spoofceleb_${split}" ]; then
            log "Making Kaldi style files for ${split}"
            mkdir -p "${trg_dir}/spoofceleb_${split}"
            python3 local/dev_data_prep.py --spoofceleb_root "${data_dir_prefix}/spoofceleb" --target_dir "${trg_dir}/spoofceleb_${split}" --split ${split}
            for f in wav.scp spk2enroll trial_label utt2spk; do
                sort "${trg_dir}/spoofceleb_${split}/${f}" -o "${trg_dir}/spoofceleb_${split}/${f}"
            done
            utils/utt2spk_to_spk2utt.pl "${trg_dir}/spoofceleb_${split}/utt2spk" > "${trg_dir}/spoofceleb_${split}/spk2utt"
            utils/validate_data_dir.sh --no-feats --no-text --no-spk-sort "${trg_dir}/spoofceleb_${split}" || exit 1
        else
            log "${trg_dir}/spoofceleb_${split} exists. Skip making Kaldi style files for spoofceleb_${split}"
        fi
    done
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    for dataset in spoofceleb_train spoofceleb_dev spoofceleb_eval; do
        # make features
