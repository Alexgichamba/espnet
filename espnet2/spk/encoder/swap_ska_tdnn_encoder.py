# SKA-TDNN with swapped order of channelwise and frequency wise SKA
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        out = self.bn1(out)
        out = self.skcwse(out) # swapped order of channelwise and frequency wise SKA
        out = self.skfwse(out)
        out += residual
        out = self.relu(out)
        return out
...
class SwapSkaTdnnEncoder(AbsEncoder):
