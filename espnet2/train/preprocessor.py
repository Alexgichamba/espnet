...
class SpkPreprocessor(CommonPreprocessor):
    """Preprocessor for Speaker tasks (Spk).
    Also used for Spoofing-aware Speaker Verification (SASV) task.
    ...
    def __init__(
        ...
        self.spf2label = None  # a dictionary that maps string spoofing label to int
        self.sample_rate = sample_rate
        self.target_duration = int(target_duration * sample_rate)
        self.num_eval = num_eval
        self.task = "spk"
        self.spf2utt = spf2utt
        self.pmos_labels = pmos_labels
        ...
        if train:
            ...
            if spf2utt is not None:
                self.task = "sasv"
                with open(spf2utt, "r") as f_sf2u:
                    self.spf2utt = f_sf2u.readlines()
            else:
                self.spf2utt = None
            if pmos_labels is not None:
                self.task = "pmos"
                with open(pmos_labels, "r") as f_pmos:
                    self.pmos_labels = f_pmos.readlines()
            else:
                self.pmos_labels = None
    ...
    def __repr__(self):
        ...
        if self.pmos_labels is not None:
            msg += f", len(utt2pmos)={len(self.pmos_labels)}"
    ...
    def _text_process(
        ...
        """Make speaker and spoof labels into integers."""
        if self.train:
