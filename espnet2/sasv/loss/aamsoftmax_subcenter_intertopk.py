...
class ArcMarginProduct_intertopk_subcenter(AbsLoss):
    ...
    def __init__(
        ...
        # initial classifier
        self.class_centers = nn.Linear(nout, nclasses * K, bias=False)
        nn.init.xavier_uniform_(self.class_centers.weight)
    ...
    def forward(self, input, label):
        # send weight to the same device as input
        self.class_centers = self.class_centers.to(input.device)

        if len(label.size()) == 2:
            label = label.squeeze(1)
        cosine = F.linear(
            F.normalize(input), F.normalize(self.class_centers.weight)
        )  # (batch, out_dim * k)
