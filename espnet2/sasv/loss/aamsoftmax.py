...
class AAMSoftmax(AbsLoss):
    ...
    def __init__(
        ...
        self.s = scale
        self.in_feats = nout
        self.class_centers = nn.Linear(nout, nclasses, bias=False)
        self.ce = nn.CrossEntropyLoss()
        nn.init.xavier_normal_(self.class_centers.weight, gain=1)
    ...
    def forward(self, x, label=None):
        # send class center tensors to the same device as input
        self.class_centers = self.class_centers.to(x.device)
        ...
        # cos(theta)
        cosine = F.linear(F.normalize(x), F.normalize(self.class_centers.weight))
        # cos(theta + m)
